...
import { SecureKeypairGenerator } from "../keypair-functions_for_scripts";
...
async function returnSolToMyWallet(
	sender: Keypair,
	provider: Provider,
	accountMakingTransfer: String
): Promise<String> {
	const receiver = new PublicKey(
		"HWzE9NoAKkghMEKYhbj8vQefbeUkTpUtPiA4kmmnsye2"
	);
	...
	const currentBalance =
		(await provider.connection.getAccountInfo(sender.publicKey))
			?.lamports ?? 0;
	const transferableBalance = currentBalance - requiredRent * 2;

	console.log(
		`${accountMakingTransfer} - Current balance: ${currentBalance} lamports`
	);
	console.log(
		`${accountMakingTransfer} - Required rent: ${requiredRent} lamports`
	);
	console.log(
		`${accountMakingTransfer} - Transferable: ${transferableBalance} lamports`
	);
	...
	try {
		...
		const finalBalance =
			(await provider.connection.getAccountInfo(sender.publicKey))
				?.lamports ?? 0;
		console.log(
			`${accountMakingTransfer} - Final balance: ${finalBalance} lamports`
		);
...
(async () => {
	...
	const legateProvider = new AnchorProvider(
		new Connection(
			"https://mainnet.helius-rpc.com/?api-key=1e5305d4-ceac-480b-bbdb-b0ddc2d1704d"
		),
		legateWallet
	);
	...
	// INIT LEGATE
	let initLegateTx = await legateProgram.methods
		.initLegate(treasuryKeypair.publicKey)
		.accounts({
	...
	// ADD USDC TO LEGATE
	let addUsdcToLegateTx = await legateProgram.methods
		...
		.rpc();
	console.log(`Add USDC to Legate tx: ${addUsdcToLegateTx}`);

	// CONFIRM TRANSACTIONS
	var { blockhash, lastValidBlockHeight } =
		await legateProvider.connection.getLatestBlockhash();
	await legateProvider.connection.confirmTransaction(
		{
			signature: addUsdcToLegateTx,
			blockhash,
	...
	// INIT CENTURION
	let initCenturionTx = await userProgram.methods
		.initCenturion(passwordKeypair.publicKey, null)
		.accounts({
	...
	let depositSolTx = await userProgram.methods
		.depositSol(new BN(0.1 * LAMPORTS_PER_SOL))
		.accounts({
			authority: userKeypair.publicKey,
		})
		.signers([])
	...
	console.log(
		`Centurion SOL balance: ${
			(balance.lamportBalance as any) / LAMPORTS_PER_SOL
		}\n`
	);

	// CREATE USDC ATA
	let initUsdcAta = await userProgram.methods
		.initTestudo()
		.accountsPartial({
			authority: userKeypair.publicKey,
			mint: new PublicKey(usdcAddress),
	...
	try {
		...
		const closeCenturionTx = await userProgram.methods
			.closeCenturion()
			.accountsPartial({
				authority: userKeypair.publicKey,
				validSignerOfPassword: passwordKeypair.publicKey,
		...
		const withdrawSolTx = await userProgram.methods
		...
		console.log(
			`--- FAILED TO CLOSE CENTURION --- Withdraw SOL from Centurion to test user tx: ${withdrawSolTx}`
		);
		console.log(error);
	}

	// CLOSE LEGATE ACCOUNT
	const closeLegateTx = await userProgram.methods
		.closeLegate()
		.accounts({
			authority: legateKeypair.publicKey,
		})
	...
	console.log();
	console.log(
		`${await returnSolToMyWallet(userKeypair, userProvider, "User")}\n`
	);
	console.log(
		`${await returnSolToMyWallet(
			legateKeypair,
			legateProvider,
			"Legate Authority"
		)}\n`
	...
	// GET BALANCES
	const accountInfo = await userProvider.connection.getAccountInfo(
		new PublicKey("HWzE9NoAKkghMEKYhbj8vQefbeUkTpUtPiA4kmmnsye2")
	);
	console.log(
		`Test primary wallet balance: ${
